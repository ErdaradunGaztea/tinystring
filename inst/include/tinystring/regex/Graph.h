...
public:
    ...
    [[nodiscard]] bool match(const TinyString& string) const;
...
private:
    [[nodiscard]] std::unordered_set<std::shared_ptr<State>> follow_epsilons(const std::shared_ptr<State>& state) const;
};
...
inline bool Graph::match(const TinyString& string) const {
    ...
    for (const auto& letter : string) {
        ...
        for (const auto& rule : rules_) {
            if (states.find(rule.start()) != states.end() && rule.match(letter)) {
...
inline std::vector<Rule>& Graph::rules() {
    return rules_;
}

inline const std::vector<Rule>& Graph::rules() const {
    return rules_;
}
...
inline std::unordered_set<std::shared_ptr<State>> Graph::follow_epsilons(const std::shared_ptr<State>& state) const {
    ...
    for (const auto& rule : rules_) {
        if (rule.is_epsilon() && rule.start() == state) {
